...
For this analysis, I have chosen the following key variables:

- **Mean Total Population:** The mean population provides an average representation of the prison population per county/state. This metric offers valuable insights into the overall size of the incarcerated population and helps in understanding the average impact of incarceration within different geographical regions.
```{r, echo=FALSE}
mean_total_pop
```
The mean of the total population is `r mean_total_pop_function()`

- **Median Total Population:** The median population represents the middle value of the prison population per county/state when arranged in ascending order. By utilizing the median, we can better assess the central tendency and identify areas where the distribution of incarcerated individuals might be skewed.
```{r, echo=FALSE}
median_total_pop
```
The median of the total population is `r median_total_pop_function()`

- **Standard Deviation Population:** The standard deviation measures the extent to which prison population data varies from the mean. A higher standard deviation suggests more significant variability in incarceration rates across different regions, indicating potential disparities or areas of concern.
```{r, echo=FALSE}
sd_total_pop
```
The standard deviation of the total population is `r sd_total_pop_function()`

- **Maximum Total Population:** The maximum population provides the highest value of the prison population per county/state. This variable allows us to identify areas with the most substantial incarceration rates and understand the scale of impact in those regions.
```{r, echo=FALSE}
max_total_pop
```
The max of the total population is `r max_total_pop_function()`

- **Minimum Population:** The minimum population gives the lowest value of the prison population per county/state. By examining the minimum, we can identify regions with relatively lower incarceration rates, potentially indicating areas where reform efforts or successful interventions have been implemented.
```{r, echo=FALSE}
min_total_pop
```
The min of the total population is `r min_total_pop_function()`
